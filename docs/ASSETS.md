...
換素材時，編輯器下方有一根「形狀」滑桿，可以把上傳的圖切成圓形或圓角矩形，**切完的形狀會直接寫進檔案**。

- **0%** ＝ 方形（不切）
- **中間值** ＝ 圓角矩形，數字是「圓角半徑佔短邊的百分比」
- **50%** ＝ 正方形素材切成正圓；長方形素材切成膠囊（兩端半圓）

### 為什麼需要這個

app 只幫 5 個位置切形狀（小幫手頁頭像、記帳付款人兩顆頭像＝圓形；記帳總額卡、側邊選單旅程卡、行程卡背景＝圓角）。**其餘 17 格 app 完全不切**，包含手機桌面的 app 圖示。現有的 Kitty 素材看起來是圓的，是因為那些 PNG 本身就去背了，不是 app 切的——所以換上一張不透明的方形照片，畫面上就會是一塊實心方塊。這根滑桿就是用來解決這件事。

### 預設值

app 本來就會切形狀的那幾格，滑桿會預先帶好對應的值（三顆頭像預設 50%＝正圓），其餘預設 0%。預設只是起點，隨時可以自己拖。

### 兩個注意事項

- **滿版啟動背景（BG_Loading）不能切**，滑桿會是灰的——滿版背景切了角會露出透明邊。
- **手機桌面圖示切圓角要小心**：iOS 不支援桌面圖示透明，切出來的透明角在 iPhone 上可能顯示成黑色（Android 正常）。工具會在你拖動滑桿時提醒。
