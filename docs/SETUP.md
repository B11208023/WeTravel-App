...
1. 打開解壓出來的資料夾，**雙擊工具**：
   - **Windows**：「`換素材工具.bat`」
   - **Mac**：「`換素材工具-Mac.command`」。第一次可能跳出「無法驗證開發者」→ 對這個檔案按**右鍵 → 打開 → 再按打開**（只需這一次）；若雙擊變成用文字編輯器打開它，改走下面的「Mac 備用方式」
...
> **Mac 備用方式（雙擊不成功時走這條）**：打開「終端機」App，先輸入 `cd ` （c、d、**再一個空格**），
> 把解壓出來的**整個資料夾**拖進終端機視窗（路徑會自動填好），按 Enter；接著貼上這行按 Enter：
> ```
> node tools/start.mjs
> ```
> 第一次會自動安裝元件（約 1 分鐘），完成後瀏覽器就會自動打開素材牆。要停止工具按 `Control + C`。

> **卡關對照**：終端機出現 `Cannot find package 'sharp'` 或 `ERR_MODULE_NOT_FOUND`，代表**元件還沒安裝**
> （直接跑 `node gui.mjs` 會這樣）。照上面用 `node tools/start.mjs` 即可，它會自己補裝。
