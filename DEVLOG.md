...
- **上游同步後必做**：在本 repo 內獨立跑 `cd tools && npm test` ＋ `node check.mjs`（不能只比對檔案一致），並對同步檔做金鑰殘留掃描（`AIza…`／`firebaseapp.com`／`…:web:` 樣式）。
...
## 2026-07-22（形狀裁切上游同步 ✅＋素材還原）
- **形狀裁切**（上游 spec/plan 產出，本 repo 同步 `f1259e5`）：換素材時多一根「形狀」滑桿（0–50%，半徑佔短邊百分比），可把上傳的圖切成圓形／圓角矩形並寫進檔案。
- 起因＝**app 只幫 5 格切形狀**（三顆頭像正圓、記帳總額卡／側邊選單旅程卡／行程卡背景圓角），**其餘 17 格完全不切**（含桌面 icon，CSS 管不到）；現行素材看起來是圓的其實是 PNG 本身去背，所以換上不透明方形照片就是一塊實心方塊。
- 實作：`assets-spec.mjs` 加 `shape` 欄位（每格預設半徑）；`gui.mjs` 的 `replaceAsset` 收 `radius`，在裁切後／`encodeToSpec` 之前用 SVG 圓角矩形 `dest-in` 合成（借量化路徑的 tRNS 保透明）；`gui.html` 加滑桿＋即時圓角預覽＋桌面圖示 iOS 透明角警告。
- **半徑公式 `r = radius/100 * min(寬,高)`，50 ⇒ 短邊一半 ⇒ 正圓。勿乘 2**——SVG `rx` 上限為邊長一半，乘 2 會讓 25%–50% 全部夾限成同一個正圓。
- 同步後在本 repo 內獨立驗證：`npm test` **28 PASS**、`check.mjs` 24 張綠、六個同步檔金鑰掃描零殘留。
- **素材還原**（`10ac5f3`）：先前上傳的 22 張＋icon 為測試圖，上游已還原，此處同步跟進，位元組與上游逐檔一致；`sw.js` CACHE_NAME v78→v79 往前 bump（不退回舊號）。素材政策不變（見檔頭，維持不換、此項已結案）。
